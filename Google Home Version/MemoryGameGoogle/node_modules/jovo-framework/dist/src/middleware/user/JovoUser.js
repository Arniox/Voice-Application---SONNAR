...
class JovoUser {
    constructor(config) {
        ...
        this.loadDb = async (handleRequest, force = false) => {
            ...
            if (handleRequest.app.$db.needsWriteFileAccess &&
                !handleRequest.host.hasWriteFileAccess) {
                return Promise.resolve();
            }
            ...
            if (this.config.sessionData && this.config.sessionData.enabled) {
                const serializedSessionData = _get(data, `${this.config.columnName}.sessionData`, {});
            ...
            jovo_core_1.Log.yellow().debug('this.$user.$sessionData');
            jovo_core_1.Log.yellow().debug(JSON.stringify(handleRequest.jovo.$user.$sessionData, null, '\t'));
        };
        this.saveDb = async (handleRequest, force = false) => {
            ...
            const updatedAt = this.config.updatedAt
    ...
    updateSessionData(handleRequest) {
        ...
        handleRequest.jovo.$user.$sessionData =
            handleRequest.jovo.$session.$data || {};
    }
}
