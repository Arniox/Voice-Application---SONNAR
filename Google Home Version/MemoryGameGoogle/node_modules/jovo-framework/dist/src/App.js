...
if (process.argv.includes('--log-level')) {
    process.env.JOVO_LOG_LEVEL = jovo_core_1.Logger.getLogLevelFromString(process.argv[process.argv.indexOf('--log-level') + 1].trim()) + '';
}
...
class App extends jovo_core_1.BaseApp {
    constructor(config) {
        ...
        if (jovo_core_1.Logger.isLogLevel(jovo_core_1.LogLevel.VERBOSE)) {
            ...
            try {
                const packageLockJson = require(pathToPackageJson); // tslint:disable-line
    ...
    async handle(host) {
        if (host.headers && host.headers['jovo-test']) {
            ...
            if (this.$db &&
                this.$db.config &&
                this.$db.config.pathToFile) { // tslint:disable-line
